        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
                    ...
                    children: [
                      Icon(
                        ...
                        color: item.folderId == null
                            ...
                            : folderColorFor(
                                ref.read(folderControllerProvider).value,
                                item.folderId,
                              ),
                      ),
              ...
              Text(
                languageByCode(item.sourceLanguageCode).name,
                ...
                child: Column(
                  children: [
                    ListTile(
                      ...
                      onTap: () => _pickVoice(context, ref, item),
                    ),
                    ...
                    ListTile(
                      ...
                      trailing: const Icon(Icons.chevron_right_rounded),
                      onTap: () => _moveToFolder(context, ref, item),
                    ),
                    const Divider(height: 1),
                    ListTile(
                      ...
                      trailing: const Icon(Icons.chevron_right_rounded),
                      onTap: () => _setReminder(context, ref, item),
                    ),
                    const Divider(height: 1),
                    ListTile(
...
Future<void> _setReminder(
  ...
  await ref
      ...
      .setReminder(item.id, reminderAt);
  if (context.mounted) {
...
Future<void> _pickVoice(
  ...
  final language = languageByCode(item.sourceLanguageCode);
  final currentVoice = ref
      .read(settingsControllerProvider)
      .value
      ?.ttsVoiceByLanguage[item.sourceLanguageCode];

  final result = await context.push<String>(
    '/voice-picker',
    extra: {
      'languageCode': language.localeHint,
  ...
  await ref
      ...
      .setTtsVoice(item.sourceLanguageCode, result.isEmpty ? null : result);
  if (!context.mounted) return;
  AppSnackbar.show(
    context,
    result.isEmpty
        ? 'Using the device default voice for ${language.name}'
        : 'Voice updated for ${language.name}',
  );
}
...
Future<void> _moveToFolder(
  ...
  await ref
      .read(historyControllerProvider.notifier)
      .moveToFolder(item.id, newFolderId);
  if (context.mounted) {
    Navigator.pop(context);
    AppSnackbar.show(
      context,
